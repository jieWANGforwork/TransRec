...
class BertAttention(nn.Module):
    def __init__(self, config,adapter=False, position_embedding_type=None):
        super().__init__()
    ...
    def forward(
        ...
        #print('outputs',self_outputs[0].size())
...
class BertModel(nn.Module):
    ...
    def forward(
        ...
        extended_attention_mask = extended_attention_mask.to(dtype=torch.float, device=device)  # fp16 compatibility
        extended_attention_mask = (1.0 - extended_attention_mask) * -10000.0 #mask的位置变负无穷，非mask位置为0
